...
function paymentMethodLabel(method: NonNullable<PublicOrder["payment"]>["method"]) {
...
export function formatShippingDisclosure(order: PublicOrder) {
  ...
  if (order.shipping.provider === "internal-fixed") return `${serviceName} · Fixed shipping rate`;
...
export function OrderDetail({ order, heading = "Order details.", showPaymentGuidance = false }: { order: PublicOrder; heading?: string; showPaymentGuidance?: boolean }) {
  ...
  const taxIncluded = order.totals.totalGstCents > 0;
  const totalLabel = taxIncluded ? "Total incl GST" : "Total";
  return <article className={styles.orderDetail}>
    ...
    <div className={styles.orderContent}>
      ...
      return <article className={styles.orderItemSnapshot} key={`${item.productTitle}-${index}`}><h3>{item.productTitle} × {item.quantity}</h3>{item.galleryDesign ? <div className={styles.gallerySnapshot}><Image src={item.galleryDesign.imageUrl} alt={item.galleryDesign.title} width={88} height={88} unoptimized /><div><strong>Selected design inspiration</strong><span>{item.galleryDesign.title}</span></div></div> : null}<dl><div><dt>Size</dt><dd>{item.sizeLabel}</dd></div>{item.orientation ? <div><dt>Orientation</dt><dd>{item.orientation}</dd></div> : null}{item.peoplePets > 0 ? <div><dt>Face or Pets</dt><dd>{item.peoplePets}</dd></div> : null}<div><dt>Photo submission</dt><dd>{item.photoSubmissionMethod === "upload" ? "Upload Photos Now" : "Send Photos After Ordering"}</dd></div>{item.designText.trim() ? <div><dt>Design text</dt><dd>{item.designText}</dd></div> : null}{item.notes.trim() ? <div><dt>Design notes</dt><dd>{item.notes}</dd></div> : null}<div><dt>Production completion date</dt><dd>{item.neededDate}</dd></div><div><dt>Urgent service</dt><dd>{item.urgentServiceConfirmed ? `Confirmed · ${item.urgentWorkingDays} working ${item.urgentWorkingDays === 1 ? "day" : "days"}` : "Not requested"}</dd></div></dl>{item.bundleComponents?.map((component) => {
        const componentLabel = component.componentKey === "roll-up" ? "Roll-Up Banner" : "Wall Banner";
        return <section aria-label={`${componentLabel} customisation`} key={component.componentKey}><h4>{componentLabel} customisation</h4><dl><div><dt>Photo submission</dt><dd>{component.photoSubmissionMethod === "upload" ? "Upload Photos Now" : "Send Photos After Ordering"}</dd></div><div><dt>Photos</dt><dd>{component.photoCount} {component.photoCount === 1 ? "photo" : "photos"}</dd></div><div><dt>Additional background removal</dt><dd>{component.backgroundRemovalCount > 0 ? `Yes · ${component.backgroundRemovalCount}` : "No"}</dd></div>{component.designText.trim() ? <div><dt>Design text</dt><dd>{component.designText}</dd></div> : null}{component.notes.trim() ? <div><dt>Design notes</dt><dd>{component.notes}</dd></div> : null}</dl></section>;
      })}{chargedLines.length ? <><h4>Price breakdown</h4><dl>{chargedLines.map((line) => <div key={line.key}><dt>{line.key === "people-pets" ? "Face or Pets fee" : line.label}</dt><dd>{amount(getPriceLineAmountInclGstCents(line))}{taxIncluded ? " incl GST" : ""}</dd></div>)}</dl></> : null}<dl><div><dt>{taxIncluded ? "Line total incl GST" : "Line total"}</dt><dd>{amount(item.lineTotalInclGstCents)}</dd></div></dl></article>;
      })}</section>
      <aside className={styles.orderTotals}><p className={styles.eyebrow}>Order total</p><h2>Order summary</h2><dl className={styles.priceLines}><div><dt>{taxIncluded ? "Products incl GST" : "Products"}</dt><dd>{amount(order.totals.productTotalInclGstCents)}</dd></div><div><dt>{taxIncluded ? "Shipping incl GST" : "Shipping"}</dt><dd>{amount(order.shipping.amountInclGstCents)}</dd></div>{taxIncluded ? <div><dt>Includes GST</dt><dd>{amount(order.totals.totalGstCents)}</dd></div> : null}{order.payment ? <><div><dt>Payment method</dt><dd>{paymentMethodLabel(order.payment.method)}{order.payment.isTest ? " (test)" : ""}</dd></div><div><dt>Payment attempt</dt><dd>{paymentAttemptStatusLabels[order.payment.status]}</dd></div></> : null}<div className={styles.priceTotal}><dt>{totalLabel}</dt><dd>{amount(order.totals.totalInclGstCents)}</dd></div></dl><p className={styles.orderShippingDisclosure}>{formatShippingDisclosure(order)}</p></aside>
    </div>
