...
export function OrderDetail({ order, heading = "Order details.", showPaymentGuidance = false }: { order: PublicOrder; heading?: string; showPaymentGuidance?: boolean }) {
  ...
  return <article className={styles.orderDetail}>
    ...
    <div className={styles.orderContent}>
      ...
      })}{chargedLines.length ? <><h4>Price breakdown</h4><dl>{chargedLines.map((line) => <div key={line.key}><dt>{line.label}</dt><dd>{amount(getPriceLineAmountInclGstCents(line))}{taxIncluded ? " incl GST" : ""}</dd></div>)}</dl></> : null}<dl><div><dt>{taxIncluded ? "Line total incl GST" : "Line total"}</dt><dd>{amount(item.lineTotalInclGstCents)}</dd></div></dl></article>;
      })}</section>
