...
export function OrderDetail({ order, heading = "Order details.", showPaymentGuidance = false }: { order: PublicOrder; heading?: string; showPaymentGuidance?: boolean }) {
  ...
  return <article className={styles.orderDetail}>
    ...
    <div className={styles.orderContent}>
      ...
      <aside className={styles.orderTotals}><p className={styles.eyebrow}>Order total</p><h2>Order summary</h2><dl className={styles.priceLines}><div><dt>{taxIncluded ? "Products incl GST" : "Products"}</dt><dd>{amount(order.totals.productTotalInclGstCents)}</dd></div><div><dt>{taxIncluded ? "Shipping incl GST" : "Shipping"}</dt><dd>{amount(order.shipping.amountInclGstCents)}</dd></div>{taxIncluded ? <div><dt>Includes GST</dt><dd>{amount(order.totals.totalGstCents)}</dd></div> : null}{order.payment ? <><div><dt>Payment method</dt><dd>{paymentMethodLabels[order.payment.method]}{order.payment.isTest ? " (test)" : ""}</dd></div><div><dt>Payment attempt</dt><dd>{paymentAttemptStatusLabels[order.payment.status]}</dd></div></> : null}<div className={styles.priceTotal}><dt>{totalLabel}</dt><dd>{amount(order.totals.totalInclGstCents)}</dd></div></dl><p className={styles.orderShippingDisclosure}>{formatShippingDisclosure(order)}</p></aside>
    </div>
