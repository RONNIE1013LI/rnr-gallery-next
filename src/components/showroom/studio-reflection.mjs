...
export function createStoneReflection(renderer, scene, camera, floor, material) {
  ...
  const resolution = new T.Vector2(960, 540);
  ...
  material.onBeforeCompile = shader => {
    ...
    shader.fragmentShader = shader.fragmentShader.replace('#include <opaque_fragment>', `
      ...
      float gloss=.14+.14*pow(1.0-incidence,4.0);
  ...
  return {
    render() {
      ...
      const w=Math.max(960,Math.min(1280,Math.round(size.x*.78)));
      const h=Math.max(540,Math.min(900,Math.round(w*size.y/Math.max(1,size.x))));
      if(resolution.x!==w||resolution.y!==h){target.setSize(w,h);resolution.set(w,h);}
