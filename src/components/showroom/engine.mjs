...
export async function createShowroom(canvas, options={}) {
 ...
 const {plaster,trim,stone,brass,aluminium,black,wood,linen}=palette;
 ...
 function label(x,y,z,w,title,detail='',parent=scene){
  ...
  const m=mesh(new T.PlaneGeometry(w,w*.174),basic(map,{transparent:true,depthWrite:false}),parent,false);m.position.set(x,y,z);return m;
 }
 ...
 scene.add(new T.HemisphereLight('#f5f5f1','#d9d4c9',1.20));
 // Four window samples create penumbrae. Maps are static and rendered only when invalidated.
 for(let i=0;i<4;i++){
  const key=new T.DirectionalLight('#fff7e9',.28);key.position.set(-4.5+(i%2)*2.5,5.5,4.8+Math.floor(i/2)*2);
 ...
 const fill=new T.DirectionalLight('#e6e9ed',.26);fill.position.set(6,3.4,4);scene.add(fill);
 for(const x of [-5.17,-1.28,2.10,4.9]){
  const spot=new T.SpotLight('#fff3de',9,9,.75,.95,2);spot.position.set(x,4.03,-.8);spot.target.position.set(x,1.7,-3.2);scene.add(spot,spot.target);
 }
 ...
 const wallAO=paint(768,256,c=>{
  ...
  let g=c.createLinearGradient(0,0,0,256);g.addColorStop(0,'rgba(0,0,0,.22)');g.addColorStop(.1,'rgba(0,0,0,0)');g.addColorStop(.82,'rgba(0,0,0,0)');g.addColorStop(1,'rgba(0,0,0,.37)');c.fillStyle=g;c.fillRect(0,0,768,256);
  for(const [cx,cy,w,h] of [[-1.28,2.1,3.7,1.85],[2.1,2.13,2.18,1.54]]){
 ...
 const wallMaterial=plaster.clone();wallMaterial.name='plaster-with-contact-occlusion';wallMaterial.aoMap=wallAO;wallMaterial.aoMapIntensity=.9;materials.add(wallMaterial);
 const wallGeometry=new T.PlaneGeometry(15,4.5);wallGeometry.setAttribute('uv1',wallGeometry.attributes.uv.clone());
 const wallFace=mesh(wallGeometry,wallMaterial,scene,false);wallFace.position.set(0,2.25,-3.215);
 ...
 const neutral=document.createElement('canvas');neutral.width=neutral.height=1;
 neutral.getContext('2d').fillStyle='#c9b99a';neutral.getContext('2d').fillRect(0,0,1,1);
 const reference=new T.CanvasTexture(neutral);reference.colorSpace=T.SRGBColorSpace;
 reference.userData.authentic=false;textures.add(reference);
 ...
 label(-1.28,.90,-3.195,1.62,'01 / Wall banner','Made for your milestone.');
 label(2.10,1.06,-3.19,1.62,'02 / Canvas','Your photographs. One family piece.');
 contact(4.9,-1.90,1.52,.75,.20);
 // Keep the physical aluminium cassette and its feet unobscured by labels.
 label(4.9,2.83,-2.99,1.50,'03 / Roll-up banner','Stand out. Celebrate.');
 referenceStatus=options.materialReference===false?'offline-neutral-back':'loading';
 if(options.materialReference!==false){
   const url=options.materialReference||'/canvas-3d/material-reference.jpg';
   load(url).then(loaded=>{
     if(disposed){loaded.dispose();return;}
     reference.image=loaded.image;reference.needsUpdate=true;reference.userData.authentic=true;
     for(const texture of textures)if(texture.source===reference.source)texture.needsUpdate=true;
     products.get('canvas').root.getObjectByName('R&R manufacturing label').visible=true;
     referenceStatus='authentic-site-reference';invalidate();
   }).catch(()=>{referenceStatus='unavailable-neutral-back';});
 }
 ...
 // Sparse olive branches and slender curved leaves, not sphere-shaped foliage.
 const tree=new T.Group();tree.position.set(6.0,0,-2.0);scene.add(tree);
 const potProfile=[new T.Vector2(.23,0),new T.Vector2(.26,.035),new T.Vector2(.30,.53),new T.Vector2(.30,.575),new T.Vector2(.285,.592),new T.Vector2(.27,.57),new T.Vector2(.25,.07)];
 mesh(new T.LatheGeometry(potProfile,48),palette.ceramic,tree);
 cylinder(0,.545,0,.263,.018,bodyMat('#494338',1),tree,.263);
 function branch(a,b,r1,r2){const mid=a.clone().add(b).multiplyScalar(.5),g=new T.CylinderGeometry(r2,r1,a.distanceTo(b),9),m=mesh(g,wood,tree);m.position.copy(mid);m.quaternion.setFromUnitVectors(new T.Vector3(0,1,0),b.clone().sub(a).normalize());}
 branch(new T.Vector3(0,.55,0),new T.Vector3(.055,2.07,.04),.026,.009);
 const leafGeo=new T.SphereGeometry(1,8,5);geometries.add(leafGeo);
 const leafMat=bodyMat('#596345',.91);leafMat.name='olive-leaf';leafMat.side=T.DoubleSide;
 const leaves=new T.InstancedMesh(leafGeo,leafMat,216);tree.add(leaves);leaves.castShadow=true;leaves.receiveShadow=true;
 const dummy=new T.Object3D();let leafIndex=0;
 for(let i=0;i<12;i++){
  const a=i*2.399,base=new T.Vector3(.02,1.02+(i%6)*.145,0),tip=new T.Vector3(Math.cos(a)*(.36+(i%3)*.11),1.28+(i%6)*.15,Math.sin(a)*.36);
  branch(base,tip,.010,.0028);
  for(let j=0;j<18;j++){
   const t=.25+j/24,side=j%2?1:-1,point=base.clone().lerp(tip,t);
   point.x+=Math.sin(j*31+i*7)*.15;point.z+=Math.sin(j*13+i*9)*.12;point.y+=Math.sin(j*19+i)*.14;
   dummy.position.copy(point);dummy.rotation.set(j*1.7,a+j*.7,-.6+Math.sin(j)*.5);dummy.scale.set(.059+(j%3)*.009,.005,.021);dummy.updateMatrix();leaves.setMatrixAt(leafIndex++,dummy.matrix);
  }
 }
 ...
 function pose(id){
  ...
  if(id==='overview'){
   ...
   return {eye:[0,1.65,Math.max(7.0,fitDistance(14.35,4.5,camera.fov,aspect,1.02)-2.9)],target:[0,2.0,-2.9]};
  }
  const ex=exhibits.get(id);if(!ex)throw new Error('Unknown showroom destination.');
  const yaw=id==='rollup'?.05:id==='welcome'?.045:0;
  const d=Math.max(id==='rollup'?4.30:2.1,fitDistance(ex.width,ex.height,camera.fov,aspect,1.27))-dolly;
  return {eye:[ex.centre.x+Math.sin(yaw)*d+(id==='canvas'?.42:0),1.65,ex.centre.z+Math.cos(yaw)*d],target:ex.centre.toArray()};
 }
