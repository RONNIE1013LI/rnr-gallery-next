...
/** Shared production/test camera poses: fixed focal length, physical camera travel. */
export function overviewPose(fov,aspect,dolly=0){
 const target=[-.40,L.overview.targetY,-2.9];
 const eye=[.15,1.65,Math.max(5.5,fitDistance(14.35,4.5,fov,aspect,1.02)-5.38)];
 const length=Math.hypot(...eye.map((value,index)=>value-target[index]));
 return {eye:eye.map((value,index)=>value-(value-target[index])/length*dolly),target};
}
export function focusPose(id,ex,fov,aspect,dolly=0){
 const yaw=id==='rollup'?.05:id==='welcome'?.045:0;
 // Reserve room for the complete cassette/feet and controls, not just the printed face.
 const padding=id==='rollup'?L.rollup.focusPadding:1.27;
 const d=Math.max(id==='rollup'?4.30:2.1,fitDistance(ex.width,ex.height,fov,aspect,padding))-dolly;
 // Approach canvas from the clear aisle, not through the newly closer roll-up.
 const offset=id==='canvas'?L.canvas.focusOffsetX:id==='banner'?L.banner.focusOffsetX:0;
 const eyeY=id==='banner'||id==='canvas'?ex.centre.y:1.65;
 return {eye:[ex.centre.x+Math.sin(yaw)*d+offset,eyeY,ex.centre.z+Math.cos(yaw)*d],target:[ex.centre.x,ex.centre.y,ex.centre.z]};
}
