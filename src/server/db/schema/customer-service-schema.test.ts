...
describe("customer service schema contract", () => {
  ...
  it("exports all eighteen Production-applied customer service tables missing from main", () => {
    expect([
      customerServiceTurns,
      customerServiceConversationEvents,
      customerServiceHumanReplyMatches,
  ...
  it("matches the applied column sets for the three extended customer service tables", () => {
    expect(getTableConfig(customerServiceConversations).columns.map((column) => column.name)).toEqual([
      "id",
      "channel",
      "external_key_hash",
      "anonymized_at",
    ...
    expect(getTableConfig(customerServiceMessages).columns.map((column) => column.name)).toEqual([
      "id",
      "conversation_id",
      "channel",
      "external_message_key_hash",
      "direction",
      "body",
      "customer_text",
      "product_context",
    ...
    expect(getTableConfig(customerServiceAiAttempts).columns.map((column) => column.name)).toEqual([
      "id",
      "message_id",
      "attempt_number",
      "trigger",
