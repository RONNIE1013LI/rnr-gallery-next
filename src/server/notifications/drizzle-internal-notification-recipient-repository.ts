...
import { createInternalNotificationVerificationLifecycleId } from "./internal-notification-verification-email";
  ...
  return Object.freeze({
    ...
    async verify(input: Parameters<InternalNotificationRecipientRepository["verify"]>[0]) {
      return database.transaction(async (transaction) => {
        ...
        if (
          !recipient ||
          !recipient.verificationTokenDigest ||
          !recipient.verificationExpiresAt ||
          recipient.verificationExpiresAt <= input.now
        ) {
          return null;
        }
        const verificationLifecycleId = createInternalNotificationVerificationLifecycleId(
          recipient.verificationTokenDigest,
        );
        ...
        await transaction.insert(adminAuditLogs).values(buildAuditRecord({
          ...
          result: "success",
          idempotencyKey: `verified:${recipient.id}:${verificationLifecycleId}`,
        }));
    ...
    async replaceSubscriptions(input: Parameters<InternalNotificationRecipientRepository["replaceSubscriptions"]>[0]) {
      try {
        return await database.transaction(async (transaction) => {
          ...
          if (replayId) {
            if (replayId !== recipient.id) {
              throw new InternalNotificationRecipientConflictError(
                "Subscription request already processed for another recipient",
              );
            }
            const replay = await loadView(transaction, recipient.id);
        ...
        if (isUniqueViolation(error)) {
          throw new InternalNotificationRecipientConflictError("Subscription request already processed");
        }
        throw error;
      }
    },
      ...
      try {
        return await database.transaction(async (transaction) => {
          ...
          if (replayId) {
            if (replayId !== recipient.id) {
              throw new InternalNotificationRecipientConflictError(
                "Disable request already processed for another recipient",
              );
            }
            const replay = await loadView(transaction, recipient.id);
            if (replay) return replay;
        ...
        if (isUniqueViolation(error)) {
          throw new InternalNotificationRecipientConflictError("Disable request already processed");
        }
        throw error;
      }
    },
