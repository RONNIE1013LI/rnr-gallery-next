...
export class MemoryWebsiteRedis implements WebsiteRedisTransport {
  ...
  async zrange(key: string, start: number, stop: number) {
    return [
      ...(key.endsWith(":activity")
        ? this.index
        : (this.indexes.get(key) ?? new Map<string, number>())),
    ]
      .sort((a, b) => b[1] - a[1])
      .slice(start, stop + 1)
      .map(([id]) => id);
  ...
  async eval(_script: string, keys: string[], args: string[]) {
    ...
    for (const [key, expected] of input.checks)
      if ((this.values.get(key) ?? null) !== expected) return 0;
    if (input.deadline && input.deadline <= input.now) return -2;
    for (const rate of input.rates) {
      const events = (this.rates.get(rate.key) ?? []).filter(
        (t) => t > input.now - rate.window,
      );
      if (events.length >= rate.limit) return -1;
    }
    for (const rate of input.rates)
      this.rates.set(rate.key, [
        ...(this.rates.get(rate.key) ?? []).filter(
          (t) => t > input.now - rate.window,
        ),
        input.now,
      ]);
    ...
    for (const [field, key] of [
      ...
      const op = input[field];
      if (op) {
        const index = this.indexes.get(key) ?? new Map<string, number>();
        if (op.score === null) index.delete(op.id);
        else index.set(op.id, op.score);
...
export function fixture() {
  ...
  const event = (
    key = "message",
    extras: Partial<HashedConversationEvent> = {},
  ): HashedConversationEvent =>
    ({
      channel: "website",
      role: "customer",
      identity: { kind: "website_conversation", keyHash: hash("identity") },
      externalConversationKeyHash: hash("conversation"),
      externalMessageKeyHash: hash(key),
      text: "What sizes do you offer?",
      attachments: [],
      receivedAt: new Date(clock),
      websiteRateLimit: {
        sessionKeyHash: hash("session"),
        networkKeyHash: hash("network"),
        sessionExpiresAt: new Date(clock + 604800000),
      },
      ...extras,
