...
const checkoutItemInputSchema = z.object({
  ...
  designText: z.string().max(MAX_CHECKOUT_TEXT_LENGTH),
  notes: z.string().max(MAX_CHECKOUT_TEXT_LENGTH),
  neededDate: z.iso.date(),
  urgentServiceConfirmed: z.boolean().optional(),
  productionWorkingDays: z.number().int().min(1).max(3).optional(),
  urgentFeeInclGstCents: z.number().int().nonnegative().optional(),
  configuredUnitPriceInclTaxCents: z.number().int().nonnegative().optional(),
  configuredCurrency: z.enum(["NZD", "AUD"]).optional(),
