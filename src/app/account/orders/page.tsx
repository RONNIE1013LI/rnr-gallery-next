...
export default async function AccountOrdersPage() { const session = await sessionOrRedirect(); let orders; try { orders = await createDrizzleOrderQueryRepository(getDatabase()).listByCustomer(session.user.id); } catch (error) { if (error instanceof OrderSnapshotIntegrityError) notFound(); throw error; } return <main id="main-content" className={styles.orderPage}><header><p className={styles.eyebrow}>Customer account</p><h1>Your orders.</h1></header>{orders.length ? <ul className={styles.orderHistory}>{orders.map((order) => <li key={order.orderNumber}><Link href={`/account/orders/${order.orderNumber}`}><strong>{order.orderNumber}</strong><span>{formatOrderDate(order.createdAt)}</span><span>{orderPaymentStatusLabels[order.paymentStatus]}</span><span>{formatNzd(order.totals.totalInclGstCents)}</span></Link></li>)}</ul> : <section className={styles.cartEmpty}><h2>No orders yet</h2><p>Your completed checkout orders will appear here.</p><Link className={styles.primaryButton} href="/shop">Explore products</Link></section>}</main>; }
