...
export default async function AdminPaymentSettingsPage({
  ...
  return <section className={`${styles.pageSection} ${styles.narrowPage}`}>
    ...
    <div className={styles.settingsGrid}>{status.providers.map((provider) => <section className={styles.panel} key={provider.key}><div className={styles.settingsHeading}><h2>{provider.label}</h2><span className={provider.enabled ? styles.enabledBadge : styles.disabledBadge}>{provider.enabled ? "Configured" : "Unavailable"}</span></div><dl className={styles.stackedDefinitionList}><div><dt>Environment</dt><dd>{provider.environment}</dd></div>{provider.market ? <div><dt>Market</dt><dd>{provider.market}</dd></div> : null}</dl></section>)}</div>
