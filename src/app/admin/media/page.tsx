...
export default async function AdminMediaPage() {
  ...
  return <section className={styles.pageSection}>
    ...
    <section className={styles.panel}><div className={styles.panelHeading}><h2>Storefront assets</h2><span>{media.storefront.length}</span></div>{media.storefront.length ? <div className={styles.mediaGrid}>{media.storefront.map((file) => <article key={file.url}><div><Image src={file.url} alt="" width={180} height={135} /></div><strong>{file.name}</strong><small>{formatBytes(file.sizeBytes)}{file.usedBy?.length ? ` · ${file.usedBy.join(", ")}` : " · Static page asset"}</small></article>)}</div> : <p className={styles.mutedText}>No images were found under public/media.</p>}</section>
    <section className={styles.panel}><div className={styles.panelHeading}><h2>Managed Design Gallery assets</h2><Link href="/admin/design-gallery">Manage gallery</Link></div>{media.gallery.length ? <div className={styles.mediaGrid}>{media.gallery.map((design) => <article key={design.id}><div><Image src={design.imageUrl} alt={design.altText} width={180} height={135} /></div><strong>{design.altText}</strong><small>{design.status} · {design.productTypeSlug}</small></article>)}</div> : <p className={styles.mutedText}>No gallery designs.</p>}</section>
  </section>;
}
