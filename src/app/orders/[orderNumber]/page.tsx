...
export default async function OrderConfirmationPage({
  ...
  const proof = proofAccess ? await getOptionalCustomerProofView(order.orderNumber, proofAccess) : null;
  return <main id="main-content" className={styles.orderPage}><PurchaseTracker event={buildPurchaseEvent(order)} /><OrderDetail order={order} heading={customerOrderHeading(order.paymentStatus)} showPaymentGuidance />{proof ? <CustomerProofPanel proof={proof} /> : null}<OrderPaymentPanel orderNumber={order.paymentReference ?? order.orderNumber} paymentStatus={order.paymentStatus} payment={order.payment} orderHref={`/orders/${order.orderNumber}`} currency={order.currency} totalInclGstCents={order.totals.totalInclGstCents} /><section className={styles.orderNext}><h2>Next steps</h2><div><Link className={styles.primaryButton} href="/shop">Continue browsing</Link>{session ? <Link className={styles.secondaryButton} href="/account/orders">View account orders</Link> : <Link className={styles.secondaryButton} href="/account/sign-in">Sign in</Link>}</div></section></main>;
}
