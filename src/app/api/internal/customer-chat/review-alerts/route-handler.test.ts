...
describe("shared Redis recovery handler", () => {
  it.each([null, "Bearer wrong", `Basic ${secret}`])("rejects unauthorized work: %s", async (authorization) => {
    ...
    const response = await createWebsiteReviewAlertCronHandler({ secret, runShared })(request(authorization));
  ...
  it("runs shared recovery once with a bounded deadline and returns only a safe count", async () => {
    ...
    const response = await createWebsiteReviewAlertCronHandler({ secret, runShared, now: () => 1000 })(request());
  ...
  it("handles empty Redis queues successfully", async () => {
    const response = await createWebsiteReviewAlertCronHandler({ secret, runShared: async () => [] })(request());
    expect(response.status).toBe(200);
  ...
  it("reports Redis failure without exposing private errors or using a fallback", async () => {
    ...
    const response = await createWebsiteReviewAlertCronHandler({ secret, runShared })(request());
  ...
  it("reports a missed deadline truthfully", async () => {
    let now = 0;
    const response = await createWebsiteReviewAlertCronHandler({
      secret, now: () => now,
